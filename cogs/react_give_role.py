...
class react_give_role(commands.Cog):
    ...
    async def react(self, interaction: discord.Interaction, message_id: str, role_id: str, emoji: str):
        ...
        try:
            ...
            try:
                data[str(message_id)][emoji.split(":")[1]] = int(role_id)
            except:
                data[str(message_id)][emoji] = int(role_id)

        except:
            try:
                message = {
                    emoji.split(":")[1]: int(role_id)
                }
            except:
                message = {
                    emoji: int(role_id)
